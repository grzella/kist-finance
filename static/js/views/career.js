// Shared with #offers — both tabs open with the same "what this is" note.
...
async function renderCareer(el) {
  ...
  if (!a.headline) {
    ...
    ${aiJsonBox("analysis_career", `Prepare a long-term career analysis for me and return ONLY valid JSON: {"headline": str, "as_of": "YYYY-MM-DD", "target_role": str, "comp_levels": [{"role": str, "comp": str, "you": bool}], "money_paths": [{"tag": "A|B|C", "title": str, "verdict": str, "text": str}], "head_of_eng": str, "ai_impact": [str], "skills": [{"skill": str, "why": str}], "skills_note": str}. Interview me about my situation first.`)}</div>`;
    return;
  }
  el.innerHTML = `
  ...
  const paintEm = (em) => {
    ...
    card.innerHTML = `
      <h3>📒 Evidence & rhythm — measuring growth <span class="muted" style="font-weight:normal;font-size:.75em">(the analysis says "what", this says "how much")</span></h3>
      <div class="grid cols-2">
        <div>
          <h4>Week ${em.this_week} in 4 numbers</h4>
        ...
        <div>
          <h4>90-day plan <span class="muted" style="font-weight:normal">${plan90.length ? `${done}/${plan90.length} ✓` : ""}</span></h4>
