...
- **Keeping the forecast journal learning daily** — the self-calibration only advances when the cycle runs, and the cycle is triggered from `/api/health`, i.e. **whenever you happen to open the app**. Open it irregularly and the journal gets multi-day holes, so the bands calibrate on a patchy error history. [`forecast-daily.sh`](forecast-daily.sh) closes that: it calls the engine directly (no HTTP, no running server), refreshes quotes, runs the cycle, takes the risk-radar reading, and logs the current band coverage so the log itself tells you whether learning is on track. Schedule it once — macOS `launchd` (a `StartCalendarInterval` agent) or a cron line such as `40 7 * * * /path/to/kist/forecast-daily.sh`. Missing history can also be seeded in one shot: `market.backfill_forecasts()` replays walk-forward forecasts over past data using **only what was known on each day**, so the conformal calibration starts with real material instead of waiting a quarter.
