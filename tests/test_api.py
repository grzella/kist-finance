...
def test_analysis_property_route(client):
    # regression: /api/analysis/property replaces /api/analysis/italy_location
...
def test_fire_projection_uses_property_key(client):
    ...
    assert "property" in d          # renamed from 'italy'
    assert "italy" not in d


def test_cashflow_uses_loan_keys(client):
    d = client.get("/api/cashflow").get_json()
    assert "loan_paid_month" in d   # renamed from 'lodz_paid_month'
